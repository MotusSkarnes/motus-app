...
describe("imageFocalPoint", () => {
  ...
  it("pans both axes with fixed scale and translate", () => {
    expect(programCoverPanTranslatePercent(0, 0.5, 1.28)).toEqual({ x: 14, y: 0 });
    expect(programCoverPanTranslatePercent(0.5, 0, 1.28)).toEqual({ x: 0, y: -14 });

    expect(programCustomCoverImageStyle("https://x/hero.jpg?fx=0&fy=0.5")).toEqual({
      objectFit: "cover",
      transform: `scale(${PROGRAM_COVER_DISPLAY_PAN_SCALE}) translate(14.00%, 0.00%)`,
      transformOrigin: "50% 50%",
    });
    expect(programCustomCoverImageStyle("https://x/hero.jpg?fx=1&fy=0.5")).toEqual({
      objectFit: "cover",
      transform: `scale(${PROGRAM_COVER_DISPLAY_PAN_SCALE}) translate(-14.00%, 0.00%)`,
      transformOrigin: "50% 50%",
    });
