...
export function pickBestPersonalGoals(candidates: Array<string | undefined | null>): string {
  ...
  for (const raw of candidates) {
    ...
    if (value.includes('"stopGoals"') || value.includes('"stopGoal"')) score += 90;
    if (value.includes('"bodyMetrics"')) score += 120;
    if (value.includes('"shareBodyMetricsWithTrainer"')) score += 40;
