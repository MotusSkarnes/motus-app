...
export function TrainerLayout({
  ...
  const trainerNotificationsList = (
    <div className="max-h-[min(22rem,70vh)] overflow-y-auto space-y-2 pr-1">
  ...
  const trainerPortalProps: ComponentProps<typeof TrainerPortal> = {
    ...
    trainerHomeHeaderActions: (
      <button
        ...
        title={trainerNotificationsOpen ? "Lukk varsler" : "Varsler"}
      >
        <Bell className="h-5 w-5" aria-hidden />
        {trainerUnreadCount > 0 ? (
          <span
            className="absolute -right-0.5 -top-0.5 inline-flex h-4 min-w-4 items-center justify-center rounded-full px-0.5 text-[9px] font-bold text-white"
            style={{ backgroundColor: MOTUS.pink }}
          >
    ...
    trainerHomeNotificationsPanel: trainerNotificationsOpen ? trainerNotificationsList : null,
    workoutMode,
    startWorkoutMode,
    updateWorkoutExerciseResult,
    replaceWorkoutExerciseGroup,
        ...
        <div className="min-w-0 space-y-4 sm:space-y-5">
          ...
          {trainerTab !== "dashboard" ? (
            <Card className="motus-card p-4 sm:p-5">
              ...
              {trainerNotificationsOpen ? <div className="mt-3">{trainerNotificationsList}</div> : null}
            </Card>
          ) : null}
