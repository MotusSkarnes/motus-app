...
export function MemberPortal(props: MemberPortalProps) {
  ...
  const [stopGoalsDraft, setStopGoalsDraft] = useState<MemberStopGoal[]>([]);
  ...
  const visiblePeriodPlans = useMemo(() => {
    const todayMs = getStartOfDay(new Date(nowTimestamp)).getTime();
    return periodPlans.filter((plan) => {
      if (hiddenPeriodPlanIds.includes(plan.id)) return false;
      if (!isMemberOwnedPeriodPlan(plan, trainerPeriodPlanIds)) return true;
      const planEnd = resolvePeriodPlanPlannedDate(plan, Math.max(1, periodPlanSelectableWeekCount(plan)), "sunday");
      if (!planEnd) return true;
      return getStartOfDay(planEnd).getTime() >= todayMs;
    });
  }, [periodPlans, hiddenPeriodPlanIds, trainerPeriodPlanIds, nowTimestamp]);
  ...
  const memberAssignedPrograms = useMemo(
  ...
  const serializedStopGoalsDraft = JSON.stringify(normalizedStopGoalsDraft);
  const serializedStopGoalsFromDb = JSON.stringify(stopGoalsFromDb);
  const profileHasUnsavedChanges = useMemo(() => {
    ...
    return (
      ...
      serializedStopGoalsDraft !== serializedStopGoalsFromDb
    );
  }, [
    editableMember,
    memberNameDraft,
    ...
    serializedStopGoalsDraft,
    serializedStopGoalsFromDb,
  ]);
  ...
  const requestRecordStopGoalBreak = useCallback(
    (index: number) => {
      if (!editableMember) return;
      const source = normalizeStopGoals(stopGoalsLatestRef.current);
      const goal = source[index];
      ...
      setConfirmDialog({
        ...
        onConfirm: () => {
          ...
          const latest = normalizeStopGoals(stopGoalsLatestRef.current);
          const updated = latest.map((item, itemIndex) =>
            itemIndex === index ? recordStopGoalBreak(item, nowDate) : item,
          );
          stopGoalsLatestRef.current = updated;
          stopGoalDraftDirtyRef.current = true;
    ...
    [editableMember, nowDate, saveProfile],
  );
  ...
  useEffect(() => {
    if (!editableMember) return;
    if (
      (stopGoalDraftDirtyRef.current || stopGoalSaveInFlightRef.current) &&
      stopGoalDraftDirtyMemberIdRef.current === editableMember.id
    ) {
      return;
    }
  ...
  const todayActivityTemplate = useMemo(() => {
    if (!todayPlanEntry.trim()) return null;
    return (
      activityTemplatesForPeriodPlan.find((template) =>
        activityTemplateMatchesPeriodEntry(template, todayPlanEntry),
      ) ?? null
    );
  ...
  const homeDisplayCoverPresentation = useMemo((): {
    ...
    if (usesPhotoStyle) {
      return {
        style: programCustomCoverImageStyle(homeWorkoutProgram.imageUrl ?? homeDisplayCoverSrc),
        usesPhotoStyle: true,
      };
    }
    return { style: { objectPosition: imageObjectPositionFromSrc(src) }, usesPhotoStyle: false };
  ...
  function saveMemberWeekPlan() {
    ...
    const plan = normalizePeriodSchedulePlan({
      ...
      title: trimmedTitle,
      notes: "Egen ukeplan laget av medlem. Gjelder kun denne uken.",
    ...
    setPeriodPlanStorageRevision((value) => value + 1);
    setPeriodPlanActionStatus("Ukeplanen er lagret for én uke.");
      ...
      safeExercises.length > 0
        ? safeExercises
            .map((exercise, index) => {
              ...
              const treatAsHold = Boolean(libraryMatch && isHoldBasedExerciseCategory(libraryMatch.category));
              const prescription = formatProgramExercisePrescription(
                {
                  ...(safeExercise as ProgramExercise),
                  weight: treatAsHold && !printField(safeExercise.holdSeconds) ? "" : String(safeExercise.weight ?? ""),
                },
                index,
                safeExercises,
                exercises,
...
<body>
                                      ...
                                      <div key={`${selectedCalendarLog.id}-${result.exerciseId}-${index}`} className="rounded-lg border bg-white p-2.5" style={{ borderColor: "rgba(15,23,42,0.08)" }}>
                                        ...
                                        <div className="mt-1 text-xs text-slate-600">
                                          {result.performedDurationMinutes
                                            ? `Utført: ${result.performedDurationMinutes || "-"} min${result.performedSpeed ? ` · ${result.performedSpeed} km/t` : ""}${result.performedIncline && Number(result.performedIncline) !== 0 ? ` · ${result.performedIncline}% stigning` : ""}`
                                        ...
                                        <div className="text-[11px] text-slate-500">
                                          ...
                                          {result.plannedDurationMinutes
                                            ? `${result.plannedDurationMinutes} min${result.plannedSpeed ? ` · ${result.plannedSpeed} km/t` : ""}${result.plannedIncline && Number(result.plannedIncline) !== 0 ? ` · ${result.plannedIncline}% stigning` : ""}`
                                            : result.exerciseCategory && isHoldBasedExerciseCategory(result.exerciseCategory)
  ...
  const memberWeekPlanBuilderPanel = showMemberWeekPlanBuilder ? (
          ...
          {memberTab === "programs" ? (
            <>
              ...
              <div className="rounded-xl border bg-white p-4 sm:p-5" style={{ borderColor: "rgba(15,23,42,0.12)" }}>
                ...
                {memberWeekPlanBuilderPanel}
                {!memberHasVisiblePeriodPlan ? (
                  <div
                    className="mt-4 rounded-xl border border-amber-200/80 bg-amber-50/90 p-4 text-sm text-amber-950"
                    role="status"
                  >
                    <p className="font-semibold">Ingen aktiv ukeplan akkurat nå</p>
                    <p className="mt-1 leading-relaxed text-amber-900/90">
                      Lag en ny ukeplan for denne uken, eller hent tilbake skjulte planer nedenfor hvis du har skjult en PT-plan.
                    </p>
                    <div className="mt-3">
                      <GradientButton
                        type="button"
                ...
                <div className="rounded-xl border bg-white p-4 sm:p-5" style={{ borderColor: "rgba(15,23,42,0.12)" }}>
                  <div className="flex min-w-0 items-start gap-2">
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="mt-1 text-sm text-slate-600">Lag en plan for denne uken, legg til en plan fra Inspo, eller be PT om en periodeplan.</p>
                    </div>
                  </div>
                  <div className="mt-4">
                    <GradientButton
