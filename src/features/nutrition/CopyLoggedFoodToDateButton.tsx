...
export function CopyLoggedFoodToDateButton({ label, sourceDateKey, onCopy }: Props) {
  const [open, setOpen] = useState(false);
  const [targetDateKey, setTargetDateKey] = useState("");
  return <>
    <button type="button" className="motus-pressable inline-flex items-center gap-1 rounded-full border border-teal-200 bg-teal-50 px-2 py-1 text-xs font-semibold text-teal-800"
      onClick={(event) => { event.stopPropagation(); setTargetDateKey(""); setOpen(true); }} aria-label={`Kopier ${label} til en annen dag`}>
      <Copy className="h-3.5 w-3.5" aria-hidden /> Kopier til dato
    </button>
