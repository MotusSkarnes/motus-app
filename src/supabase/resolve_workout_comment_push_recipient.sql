...
create or replace function public.resolve_workout_comment_push_recipient(p_log_id uuid)
returns uuid
language sql
stable
security definer
...
revoke all on function public.resolve_workout_comment_push_recipient(uuid) from public;
grant execute on function public.resolve_workout_comment_push_recipient(uuid) to service_role;
